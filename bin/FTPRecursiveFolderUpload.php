...
/*echo ' 4654sd48sa7d98sD81s8d71dsa <pre>' . print_r($files, true) . '</pre>';
@ftp_close($conn_id);
exit;*/
...
if ($files)
{
	foreach ($files as $file)
	{
		...
		echo 'Transfer ' . $dir . '/' . $file . ' -> ' . ftp_pwd($conn_id) . ' ' . basename($file) . NL;
		...
		if ($success === false)
		{
			echo 'Error while ftp_put. File: ' . $dir . "/"  . $file . NL .
...
{
	echo 'No files/folders found in local ' . $dir . '.' . NL;
}
